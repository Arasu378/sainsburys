...
class BasicSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.sainsburys.co.uk/webapp/wcs/stores/servlet/gb/groceries/meat-fish/?langId=44&storeId=10151&krypto=58VPkIE3kgaqsdXsUU3nRG6mVumNTtbOLZrFIDT2yMuWiO432awFFw25yH%2Fyvy98VCfxkTPvM0BgYPotnK8WlnXWTQH0SJm0sd7NWWHALAlyZbJOF0bxTo9lXkP%2FMrCONRBEoBqRvgHAf9BKEw1jGOLD3GUZAQk8XW1zvnRdNbs%3D&ddkey=https%3Agb%2Fgroceries%2Fmeat-fish%2F']
    ...
    def parse_department_pages(self, response):
        product_grid = response.xpath('//ul[@class="product Lister gridView"]')
        if product_grid:
            for product in self.handle_product_listings(response):
                yield product
        pages = response.xpath('//ul[@class="categories shelf"]/li/a')
        if not pages:
            pages = response.xpath('//ul[@class="categories ailsles"]/li/a')
        if not pages:
